...
_rabbitmqadmin()
{
    ...
    case "${prev}" in
        list)
            COMPREPLY=( $(compgen -W 'channels vhosts users connections policies nodes queues permissions bindings consumers parameters exchanges' -- ${cur}) )
            return 0
            ;;
        show)
            ...
            ;;
        declare)
            COMPREPLY=( $(compgen -W 'permission vhost binding policy parameter queue exchange user' -- ${cur}) )
            return 0
            ;;
        delete)
            COMPREPLY=( $(compgen -W 'permission vhost binding policy parameter queue exchange user' -- ${cur}) )
        ...
        --username)
            ...
            ;;
        -f)
            COMPREPLY=( $(compgen -W "pretty_json kvp tsv long table bash raw_json"  -- ${cur}) )
            return 0
            ;;
        --format)
            COMPREPLY=( $(compgen -W "pretty_json kvp tsv long table bash raw_json"  -- ${cur}) )
